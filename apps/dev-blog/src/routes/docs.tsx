...
import { ContentEntry, getAllContent } from '../utils/content';
...
function buildTree(docs: ContentEntry[]) {
  ...
  const sortNodes = (nodes: any[]) => {
    nodes.sort((a, b) => {
      const wA =
        typeof a.doc.metadata.weight === 'number' ? a.doc.metadata.weight : 999;
      const wB =
        typeof b.doc.metadata.weight === 'number' ? b.doc.metadata.weight : 999;
      return wA - wB;
    });
...
function TreeNode(props: { node: { doc: ContentEntry; children: any[] } }) {
  ...
  const slug =
    props.node.doc.path.split('/documentation/')[1] ||
    props.node.doc.path.split('/').pop();
  ...
  return (
    <div class="flex flex-col md:flex-row gap-8">
      <aside class="md:w-64 flex-shrink-0 md:sticky md:top-24 md:max-h-[calc(100vh-8rem)] overflow-y-auto">
        <h2 class="font-bold text-lg mb-4 text-zinc-900 dark:text-zinc-100">
          Documentation
        </h2>
        <Suspense
          fallback={
            <div class="animate-pulse h-32 bg-zinc-200 dark:bg-zinc-800 rounded-lg" />
          }
        >
          <ul class="-ml-3">
